...
PickupsService.dropAllPlease = function()
	...
	if Config.DropOnRespawn.Items then
		...
		for _, item in pairs(items) do
			...
			TriggerServerEvent("vorpinventory:serverDropItemDeath", itemName, item.id, tonumber(itemCount), itemMetadata)
	...
	if Config.DropOnRespawn.Weapons then
		...
		for index, weapon in pairs(weapons) do
			TriggerServerEvent("vorpinventory:serverDropWeaponDeath", index)
